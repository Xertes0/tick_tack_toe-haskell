...
placeTile (mouseX, mouseY) state =
  if canPlace
    then state
           { board = Seq.update boardIndex tileToPlace (board state)
           , gameState = newGameState
           }
    else state
  where
    (clickX, clickY) =
      (mouseX + (windowWidth / 2.0), mouseY + (windowHeight / 2.0))
...
checkGameOver state
  | isJust winner = state {gameState = GameOver winner}
  | otherwise = state
  where
    wonInRow = chunksOf 3 $ toList $ board state
    wonInCol = transpose $ chunksOf 3 $ toList $ board state
    wonInCross =
      map (map (\x -> Seq.index (board state) (x - 1))) [[1, 5, 9], [3, 5, 7]]
    winner =
      listToMaybe
        $ mapMaybe (listWon . toList) (wonInRow ++ wonInCol ++ wonInCross)
